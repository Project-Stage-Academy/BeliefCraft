...
- Retrieve the Python source code related to algorithm or example documents, assembled in call order.
...
- `src/rag_service/repositories.py`: Abstract repository interface and implementations (e.g., `FakeDataRepository`, `WeaviateRepository`).
The repository pattern allows for easy swapping of the underlying data store without changing the MCP tool logic.
- `src/rag_service/code_entity_processor.py`: `CodeDefinitionProcessor` — stateless processor that traverses code-definition reference graphs (Weaviate objects → `Document` lists) and reconstructs ordered Python source fragments from those documents.
