...
import pytest
from database.enums import LocationType, OrderStatus, ShipmentDirection
from database.models import Location, Order, OrderLine, Product, Warehouse
from environment_api.data_generator.logic.outbound import OutboundManager
...
def outbound_manager(mock_session, mock_ledger):
    ...
    with patch(
        "environment_api.data_generator.logic.outbound.InventoryLedger", return_value=mock_ledger
    ):
...
class TestOutboundManager:
    @patch("environment_api.data_generator.logic.outbound.settings")
