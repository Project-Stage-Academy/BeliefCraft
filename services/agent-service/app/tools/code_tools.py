    ...
    def get_metadata(self) -> ToolMetadata:
        return ToolMetadata(
            ...
            description=(
                ...
                "Workflow: 1) Retrieve necessary code from RAG. 2) Retrieve context from the environment. "
                "3) Combine them into a single script. "
                "4) Pass the script to this tool. "
                "5) Evaluate 'stdout', 'stderr', and 'exit_code' to determine if the execution succeeded."
            ),
            category="utility",
            parameters={
                "type": "object",
                "properties": {
                    "code": {
                        "type": "string",
                        "description": (
                            "The executable Python script. Must be raw, valid Python code without markdown "
                            "formatting (do not wrap in ```python). Access environment variables via the "
                            "'env_data' dictionary. You MUST print the final result to stdout, as only "
                    ...
                    "data": {
                        "type": "object",
                        "description": (
                            "Environment data to be processed. Passed as a JSON object and injected "
                            "automatically as a dictionary named 'env_data' inside the script. "
                            "Do not hardcode environment variables into the 'code' string; use this parameter."
                        )
                    }
                },
                "required": ["code"],
