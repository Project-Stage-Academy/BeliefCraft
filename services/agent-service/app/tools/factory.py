...
from app.tools.orchestration_tools import CallEnvSubAgentTool, CallRAGSubAgentTool
...
class ToolRegistryFactory:
    ...
    def create_react_agent_registry(
        ...
        rag_sub_registry: ToolRegistry | None = None,
    ) -> ToolRegistry:
        ...
        # Inject the sub-agent tools into the ReAct registry
        if env_sub_registry:
            registry.register(CallEnvSubAgentTool(env_registry=env_sub_registry))
            logger.info("react_registry_loaded_env_orchestration_tool")

        if rag_sub_registry:
            registry.register(CallRAGSubAgentTool(rag_registry=rag_sub_registry))
            logger.info("react_registry_loaded_rag_orchestration_tool")
    ...
    @staticmethod
    def create_rag_sub_agent_registry(
        ...
        if mcp_rag_tools:
            for tool in mcp_rag_tools:
                registry.register(tool)
            logger.info("rag_sub_registry_loaded_mcp_tools", count=len(mcp_rag_tools))

        logger.info(
            "rag_sub_agent_registry_created",
            total_tools=len(registry.tools),
        )
