...
class CachedTool(BaseTool):
    ...
    def _generate_cache_key(self, **kwargs: Any) -> str:
        ...
        Uses MD5 hash of sorted JSON parameters to ensure:
        ...
        params_hash = hashlib.md5(params_str.encode()).hexdigest()  # noqa: S324
    ...
    async def run(self, **kwargs: Any) -> ToolResult:
        ...
        # Check if key exists before execution (for cached flag)
        try:
            is_cached = await self.redis_client.exists(cache_key)
        except Exception:
            # If Redis check fails, assume not cached
            is_cached = 0

        # Execute tool (will use cache if available)
        result = await super().run(**kwargs)
        result.cached = bool(is_cached)
