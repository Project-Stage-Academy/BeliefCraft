...
class BlockChunkBBox(BaseModel):
...
def _build_label(chunk: BlockChunkBBox) -> str:
    if chunk.entity_id and chunk.chunk_type:
        return f"{chunk.chunk_type}:{chunk.entity_id}"
    if chunk.entity_id:
        return chunk.entity_id
    return chunk.chunk_type or "block"


def _load_chunks(input_json: Path) -> list[BlockChunkBBox]:
    ...
    for index, row in enumerate(rows):
        ...
        try:
            parsed.append(BlockChunkBBox.model_validate(row))
...
def _to_pdf_rect(bbox: list[float], bbox_space: str, bbox_dpi: float) -> fitz.Rect:
    x0, y0, x1, y1 = bbox
    if bbox_space == "pixel":
        scale = 72.0 / bbox_dpi
...
def render_bboxes_to_pdf(
    ...
    by_page: dict[int, list[BlockChunkBBox]] = defaultdict(list)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Draw block bounding boxes from blocks_metadata JSON onto PDF pages. "
            "block_processor outputs bboxes in PDF points."
        ),
    )
    parser.add_argument("input_json", type=Path, help="Path to blocks_metadata JSON")
    parser.add_argument("input_pdf", type=Path, help="Path to source PDF")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        default=Path("blocks_annotated_book.pdf"),
    ...
    parser.add_argument(
        ...
        default="point",
        help=(
            "Coordinate space of JSON bbox values. Use 'point' for block_processor output "
            "(default), 'pixel' to convert with 72/dpi."
        ),
    )
    parser.add_argument(
        "--bbox-dpi",
        type=float,
        default=200.0,
        help="DPI used only when --bbox-space=pixel",
    )
    return parser
