...
class DocumentAssembler:
    def __init__(self, paddle_dir, figures_json, blocks_json, tables_json, formulas_json):
        ...
        self._validate_files([self.figures_json, self.blocks_json, self.tables_json, self.formulas_json])
    ...
    def _load_all_paddle_jsons(self, directory):
        ...
        files = sorted(
            [f for f in path_dir.iterdir() if f.suffix == '.json'],
            key=lambda f: [int(s) if s.isdigit() else s.lower() for s in re.split(r'(\d+)', f.name)]
        )
    ...
    def _merge_visual_items(self, items):
        ...
        for item in items:
            ...
            if eid not in merged: merged[eid] = item
            else:
                if "image_index" in item: merged[eid]["image_index"] = item["image_index"]
                if "bbox" in item:
    ...
    def _add_formula_chunk(self, f_id, page_num):
