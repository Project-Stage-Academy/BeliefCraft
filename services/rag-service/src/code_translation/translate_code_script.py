...
def send_prompt(client, prompt: str) -> str:
    body = {
...
def main() -> None:
    ...
    for chapter in TRANSLATED_CHAPTERS:
        ...
        with open(update_description_dir / f"chapter_{chapter}_code_translation.txt", "w",
                  encoding="utf-8") as f:
            f.write(prompt)
    ...
    for chapter in CHAPTERS_TO_TRANSLATE:
        ...
        update_description_dir.mkdir(parents=True, exist_ok=True)
        with open(update_description_dir / f"chapter_{chapter}_translation.txt", "w", encoding="utf-8") as f:
            f.write(prompt)
    ...
    for example in EXAMPLE_WITH_CODE_NUMBERS:
        ...
        with open(translate_examples_dir / f"{example.replace(' ', '_')}_translation.txt", "w", encoding="utf-8") as f:
            f.write(prompt)
        prompts_queue.append(prompt)
